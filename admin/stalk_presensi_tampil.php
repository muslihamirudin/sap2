...
<head>
    ...
    <title>S A P | Safety Talk - Presensi</title>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                 <!--  page header -->
                <div class="col-lg-12">
                    <h1 class="page-header">Presensi Safety Talk</h1>
                </div>
                 <!-- end  page header -->
            </div>
            ...
            <div class="row">
                <div class="col-lg-5">
                    ...
                    <div class="panel panel-primary">
                        ...
                        <form method="get">
                            ...
                            <select name='tahun' >
                                <?php
                                $qry=mysqli_query($koneksi,"SELECT tgl_presensi FROM stalk_presensi GROUP BY year(tgl_presensi)");
                                 while ($ar = mysqli_fetch_array ($qry)){
                                     $data = explode('-',$ar['tgl_presensi']);
                                     $tahun = $data[0];
                             ...
                             <a href="stalk_presensi_tampil.php" button type="submit" class="btn btn-success">Reset</a>
                             </form>
                                <?php
                               $dbHost='localhost';
                               $dbUsername = 'root';
                               ...
                               //  $queryarsip = mysqli_query ($conn, "select tanggal from klipping where month(tanggal)='$bln' and year(tanggal)='$thn' group by tanggal"); 
                                 ...
                                 echo '<h4> &nbsp Grafik Presensi Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</h3>';
                                 $result = $db->query("SELECT SUM(IF(ket='Hadir',1,0)) AS HADIR, SUM(IF(ket='Tidak Hadir',1,0)) AS TIDAK_HADIR FROM stalk_presensi WHERE MONTH(tgl_presensi)='$bln' AND YEAR(tgl_presensi)='$thn'");
                             }  
                             else
                             {
                                ...
                                // $queryarsip = mysqli_query ($conn, "select tanggal from klipping group by tanggal"); 
                                 $result = $db->query("SELECT SUM(IF(ket='Hadir',1,0)) AS HADIR, SUM(IF(ket='Tidak Hadir',1,0)) AS TIDAK_HADIR FROM stalk_presensi");
                             }
                  ...
                  <div class="panel panel-primary">
                     <div class="panel-heading">
                       ...
                       <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
                                <form role="form" method="POST" action="stalk_presensi_tambah_proses.php" enctype="multipart/form-data">
                                        <?php 
                                        $tgl= date('Y-m-d');
                                        ?>
                                        ...
                                        <div class="form-group">
                                            <label>Tanggal</label>
                                             <input class="form-control" name="tgl_presensi" value="<?php echo fungsi_tanggal($tgl); ?>" readonly>
                                        </div>
              ...
              <div class="row">
                <div class="col-lg-12">
                  <div class="panel panel-primary">
                     <div class="panel-heading">
                            <i class="fa fa-flag fa-fw"></i> Berikut merupakan Data Kehadiran Karyawan pada Upacara Safety Talk
                            <div class="pull-right">
                            </div>
                        </div>
                        ...
                        <script src="js/jquery.min.js"></script> <!-- Load file jquery -->
                         ...
                         <form method="get" action="">
                            <h3>Menu Cetak</h3>
                                ...
                                <option value="">Pilih</option>
                                <?php
                                $query = "SELECT YEAR(tgl_presensi) AS tahun FROM stalk_presensi GROUP BY YEAR(tgl_presensi)"; // Tampilkan tahun sesuai di tabel transaksi
                                ...
                                <a href="stalk_presensi_tampil.php" <button type="button" class="btn btn-danger waves-effect waves-light"> Reset Filter </button> </a>
                                </form>
                                <?php
                            if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
                                ...
                                if($filter == '1'){ // Jika filter nya 1 (per bulan)
                                    ...
                                    echo '<a href="stalk_presensi_cetak.php?filter=1&bulan='.$_GET['bulan'].'&tahun='.$_GET['tahun'].'" <button type="button" class="btn btn-warning waves-effect waves-light"> Cetak PDF</button></a> <br />';

                                    $query = "SELECT id_presensi, tgl_presensi, nik, nama, jabatan, departemen, ket FROM stalk_presensi, karyawan, jabatan, departemen WHERE stalk_presensi.id_karyawan=karyawan.id_karyawan AND karyawan.id_jabatan=jabatan.id_jabatan AND karyawan.id_departemen=departemen.id_departemen AND MONTH(tgl_presensi)='".$_GET['bulan']."' AND YEAR(tgl_presensi)='".$_GET['tahun']."'"; // Tampilkan data laporan sesuai bulan dan tahun yang diinput oleh user pada filter
                                }else{ // Jika filter nya 3 (per tahun)
                                    echo '<h3>Data Presensi Tahun '.$_GET['tahun'].'</h3>';
                                    echo '<a href="stalk_presensi_cetak.php?filter=2&tahun='.$_GET['tahun'].'" <button type="button" class="btn btn-warning waves-effect waves-light"> Cetak PDF</button></a> <br />';

                                    $query = "SELECT id_presensi, tgl_presensi, nik, nama, jabatan, departemen, ket FROM stalk_presensi, karyawan, jabatan, departemen WHERE stalk_presensi.id_karyawan=karyawan.id_karyawan AND karyawan.id_jabatan=jabatan.id_jabatan AND karyawan.id_departemen=departemen.id_departemen AND YEAR(tgl_presensi)='".$_GET['tahun']."'"; // Tampilkan data laporan sesuai tahun yang diinput oleh user pada filter
                                }
                            }else{ // Jika user tidak mengklik tombol tampilkan
                                echo '<h3>Semua Data Presensi </h3>';
                                echo '<a href="stalk_presensi_cetak.php" button type="button" class="btn btn-warning waves-effect waves-light"> Cetak PDF</button></a> <br /> <br />';



                                $query = "SELECT id_presensi, tgl_presensi, nik, nama, jabatan, departemen, ket FROM stalk_presensi, karyawan, jabatan, departemen WHERE stalk_presensi.id_karyawan=karyawan.id_karyawan AND karyawan.id_jabatan=jabatan.id_jabatan AND karyawan.id_departemen=departemen.id_departemen ORDER BY tgl_presensi"; // Tampilkan semua data laporan diurutkan berdasarkan tanggal
                            }
                            ?>
                             <script>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                     ...
                                     <tbody>
                                              ...
                                              while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
                                                ...
                                                echo "<tr>
                                                <td>".$no++."</td>
                                                <td>".fungsi_tanggal($data['tgl_presensi'])."</td>
                                                <td>".$data['nik']."</td>
                                                <td>".$data['nama']."</td>
                                                <td>".$data['jabatan']."</td>
                                                ...
                                                <td><a href='stalk_presensi_hapus.php?id_presensi=$data[id_presensi]' class='btn btn-danger'><i class='fa fa-trash-o'></a></td>
                                                </tr>";
                                                 }
