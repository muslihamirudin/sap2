...
<br>
 ...
 <div class="head" style="padding: 3mm; border: 1px solid;" align="center"><span style="font-size: 20px" >DATA PRESENSI SAFETY TALK</span> </div>
 	...
 	if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter
        ...
        $query = "SELECT id_presensi, tgl_presensi, nik, nama, jabatan, departemen, ket FROM stalk_presensi, karyawan, jabatan, departemen WHERE stalk_presensi.id_karyawan=karyawan.id_karyawan AND karyawan.id_jabatan=jabatan.id_jabatan AND karyawan.id_departemen=departemen.id_departemen"; // Tampilkan semua data laporan diurutkan berdasarkan tanggal
    }
    ?>
	...
	if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
		while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
			...
			echo "<tr>";
            echo "<td style= 'text-align: center; width=90px;'>".fungsi_tanggal($data['tgl_presensi'])."</td>";
...
$pdf->WriteHTML($html);
$pdf->Output('Data Presensi-SafetyTalk.pdf', 'D');
?>