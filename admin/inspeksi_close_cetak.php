...
<br>
 ...
 <div class="head" style="padding: 3mm; border: 1px solid;" align="center"><span style="font-size: 14px" >INSPEKSI CLOSE</span> </div>
 	...
 	if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter
        ...
        if($filter == '1'){ // Jika filter nya 1 (per tanggal)
            $tgl = date('d-m-y', strtotime($_GET['tgl_inscl']));

            echo '<br /> <p align="center" style="font-weight: bold; font-size: 18px;"><u>DATA LAPORAN TANGGAL '.$tgl.'</u></p>';

            $query = "SELECT id_inscl, tgl_inscl, gambar_perbaikan, tgl_insop, due_insop, jenis_insop, lokasi, desk, gambar, bahaya, rekomendasi, nama, head  FROM inspeksi_close, inspeksi_open, lokasi, bahaya, karyawan, departemen WHERE inspeksi_close.id_insop=inspeksi_open.id_insop AND inspeksi_open.id_lokasi=lokasi.id_lokasi AND inspeksi_open.id_bahaya=bahaya.id_bahaya AND inspeksi_open.inspektor=karyawan.id_karyawan AND inspeksi_open.id_departemen=departemen.id_departemen AND valid=1 AND DATE(tgl_inscl)='".$_GET['tgl_inscl']."'"; // Tampilkan data laporan sesuai tanggal yang diinput oleh user pada filter
    ...
    <table class="tabel2">
      <tr>
        ...
        <td style="text-align: center; "><b>Inspektor</b></td>
	...
	if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
		while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
			...
			echo "<tr>";
            echo "<td style= 'text-align: center; width=30px;'>".fungsi_tanggal($data['tgl_inscl'])."</td>";
            echo "<td style= 'text-align: center; width=85px;'><img src=assets/img/".$data['gambar_perbaikan']."></td>";
            echo "<td style= 'text-align: center; width=55px;'>".fungsi_tanggal($data['tgl_insop'])."</td>";
            echo "<td style= 'text-align: center; width=55px;'>".fungsi_tanggal($data['due_insop'])."</td>";
            echo "<td style= 'text-align: center; width=50px;'>".$data['jenis_insop']."</td>";
            echo "<td style= 'text-align: center; width=40px;'>".$data['lokasi']."</td>";
            echo "<td style= 'text-align: center; width=75px;'>".$data['desk']."</td>";
            echo "<td style= 'text-align: center; width=85px;'><img src=assets/img/".$data['gambar']."></td>";
            echo "<td style= 'text-align: center; width=40px;'>".$data['bahaya']."</td>";
            echo "<td style= 'text-align: center; width=80px;'>".$data['rekomendasi']."</td>";
            echo "<td style= 'text-align: center; width=50px;'>".$data['nama']."</td>";
            echo "<td style= 'text-align: center; width=50px;'>".$data['head']."</td>";
            echo "</tr>";
		}
...
$pdf->WriteHTML($html);
$pdf->Output('Data Laporan-INSPEKSI-Close.pdf', 'D');
?>